...
const ActiveOrders = ({navigation}) => {
  ...
  const [routeorders_processing] = useState([]);
  const [routeaddress] = useState([]);
  ...
  const OrdersList = () => {
    ...
    .then(function(res){
      ...
      routeorders_processing.length = 0;
  ...
  const FetchUsers = (dat) => {
    dat.map((item)=>{
      ...
      .then(snapshot => {
        ...
        orders_active = vals.orders_active,
        orders_processing = vals.orders_processing,
        orders_address = vals.address;
        routeorders.push(orders_active);
        routeorders_processing.push(orders_processing);
        routeaddress.push(orders_address);
        SetDisplay(true);
      });
    });
  };
  ...
  const CreateProcessing = (rar,res,rad,index) => {
    ...
    routeorders_processing[index].push(orderHalf);
    UpdateProcessing();
  };
  const UpdateProcessing = () => {
    routeid.map((ind,pos)=>{
      ...
      .ref('/users/'+dataId+'')
      .update({
          "orders_active" : routeorders[pos],
          "orders_processing" : routeorders_processing[pos]
      })
      .then(function(){
        SetDisplay(false);
        OrdersList();
  ...
  const OrderAddress = (res) => {
    var rad = res.ordindex, rar = res.ordpos;
    if(rar){
      var ordprop = routeaddress[rad][rar];
      return(
        <View style={HomeStyle.HomAddView}>
          <Text style={HomeStyle.HomAddViewBigText}>Shipping Details</Text>
          <Text style={HomeStyle.HomAddViewMidText}>Name : {ordprop.lastname} {ordprop.firstname}</Text>
          <Text style={HomeStyle.HomAddViewMidText}>Address 1 : {ordprop.address1}</Text>
          <Text style={HomeStyle.HomAddViewMidText}>Address 2 : {ordprop.address2}</Text>
          <Text style={HomeStyle.HomAddViewMidText}>State : {ordprop.state}</Text>
          <Text style={HomeStyle.HomAddViewMidText}>City : {ordprop.city}</Text>
          <Text style={HomeStyle.HomAddViewMidText}>Phone : {ordprop.phonenumber}</Text>
          <Text style={HomeStyle.HomAddViewMidText}>Zip Code : {ordprop.postcode}</Text>
        </View>
      );
    }
  };
  ...
  return(
    <SafeAreaView style={HomeStyle.HomSafeArea}>
    {
              ...
              var ItemRet = items.map((item,res)=>{
                          ...
                          <View style={HomeStyle.HomViewBase} key={(item.OrderId+itm.id)}>
                            ...
                            <View style={HomeStyle.HomViewBaseBase}>
                              <View style={HomeStyle.HomViewBaseBaseRow}> 
                                <Text style={[HomeStyle.HomViewBaseBaseText,{color:'#2828f4'}]}>This order is active.</Text>
                              </View>
                              <Ripple style={HomeStyle.HomAccPressable} rippleColor='#676767' rippleDuration={500} rippleOpacity={0.9} rippleSize={100} onPress={()=>AcceptOrder(itm,item,rest)}>
                                  <Text style={HomeStyle.HomAccPressableText}>Accept Order</Text>
                              </Ripple>
                            </View>
                          </View>
                        </>
                ...
                return(  
                  <>
                    ...
                    {ItemProductRet}
                    <OrderAddress ordindex={rest} ordpos={item.OrderAddressId}/>
                  </>
                  );
              })
              return ItemRet;
      ...
      <View style={HomeStyle.HomViewTabNull}>
        ...
        <Text style={HomeStyle.HomViewTabNullText}>No active orders.</Text>
      </View>
    }
    </SafeAreaView>
  );
